...
def round_to_x_digits(n,x):
    return round(n * math.pow(10, x)) / math.pow(10, x)
...
def crack_time_to_score(seconds):
    if seconds < math.pow(10, 2): return 0
    if seconds < math.pow(10, 4): return 1
    if seconds < math.pow(10, 6): return 2
    if seconds < math.pow(10, 8): return 3
    return 4
...
def calc_entropy(match):
    ...
    if match['pattern'] == 'repeat': entropy_func = repeat_entropy
    elif match['pattern'] == 'sequence': entropy_func = sequence_entropy
    elif match['pattern'] == 'digits': entropy_func = digits_entropy
    elif match['pattern'] == 'year': entropy_func = year_entropy
    elif match['pattern'] == 'date': entropy_func = date_entropy
    elif match['pattern'] == 'spatial': entropy_func = spatial_entropy
    elif match['pattern'] == 'dictionary': entropy_func = dictionary_entropy
...
def extra_uppercase_entropy(match):
    ...
    possibilities = sum(binom(U + L, i) for i in range(0, min(U,L)+1))
...
def extra_l33t_entropy(match):
    ...
    for subbed, unsubbed in match['sub'].items():
        S = len([x for x in match['token'] if x == subbed])
        U = len([x for x in match['token'] if x == unsubbed])
        possibilities += sum(binom(U + S, i) for i in range(0, min(U, S)+1))
